...
Deno.serve(async (req) => {
  ...
  try {
    // Verify JWT and extract caller identity before any data access
    ...
    // Validate caller belongs to the requested company (prevents cross-tenant data access)
    if (company_id) {
    ...
    if (analytics) {
      let q = adminClient
        .from('recognitions')
        .select('id, points, program, from_user_id, to_user_id, created_at, is_private')
    ...
    let query = adminClient
      .from('recognitions')
      .select(`
        id, points, program, message, created_at, company_id, is_private,
